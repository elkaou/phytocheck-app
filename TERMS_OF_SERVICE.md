# Conditions Générales d'Utilisation de PhytoCheck

**Dernière mise à jour : 20 février 2026**

## 1. Acceptation des conditions

En téléchargeant, installant ou utilisant l'application PhytoCheck, vous acceptez d'être lié par les présentes Conditions Générales d'Utilisation (CGU). Si vous n'acceptez pas ces conditions, veuillez ne pas utiliser l'application.

## 2. Description du service

PhytoCheck est une application mobile gratuite (avec option d'abonnement Premium) qui permet de :
- Vérifier l'homologation des produits phytosanitaires en France
- Consulter les informations issues de la base de données publique E-Phy
- Scanner les étiquettes de produits pour une identification rapide
- Gérer un stock personnel de produits phytosanitaires

## 3. Source des données

### 3.1 Base de données E-Phy

Les informations sur les produits phytosanitaires proviennent de la **base de données publique E-Phy**, gérée par l'ANSES (Agence nationale de sécurité sanitaire de l'alimentation, de l'environnement et du travail) et le Ministère de l'Agriculture et de la Souveraineté alimentaire français.

**Source officielle** : https://ephy.anses.fr/

### 3.2 Date de mise à jour

La base de données intégrée dans l'application est mise à jour périodiquement. La date de la dernière mise à jour est affichée sur l'écran d'accueil de l'application.

**Date actuelle de la base** : 21/01/2026

## 4. DÉCHARGE DE RESPONSABILITÉ

### 4.1 Outil d'information uniquement

**PhytoCheck est un outil d'information et d'aide à la décision. Il ne remplace en aucun cas la consultation de la base de données officielle E-Phy ni l'avis d'un professionnel qualifié.**

### 4.2 Vérification obligatoire

**VOUS ÊTES TENU DE VÉRIFIER TOUTES LES INFORMATIONS DANS LA BASE DE DONNÉES OFFICIELLE E-Phy (https://ephy.anses.fr/) AVANT TOUTE UTILISATION, ACHAT, VENTE OU ÉLIMINATION DE PRODUITS PHYTOSANITAIRES.**

L'application PhytoCheck ne peut être tenue pour seule source d'information pour des décisions ayant des conséquences légales, sanitaires, environnementales ou financières.

### 4.3 Exactitude des données

Bien que nous nous efforcions de maintenir les données à jour et exactes, **nous ne garantissons pas** :
- L'exactitude complète des informations affichées
- L'absence d'erreurs ou d'omissions
- La mise à jour en temps réel des données par rapport à la base E-Phy officielle
- La disponibilité continue du service

### 4.4 Limitation de responsabilité

**LE DÉVELOPPEUR DE PHYTOCHECK, FRANCK COURRÈGES, SE DÉGAGE DE TOUTE RESPONSABILITÉ CONCERNANT :**

1. **Utilisation des produits** : Toute utilisation, manipulation, stockage ou élimination de produits phytosanitaires basée sur les informations fournies par l'application
2. **Erreurs ou omissions** : Toute erreur, omission, inexactitude ou obsolescence des données
3. **Dommages** : Tout dommage direct ou indirect résultant de l'utilisation de l'application, incluant mais non limité à :
   - Dommages matériels
   - Dommages corporels
   - Dommages environnementaux
   - Pertes financières
   - Sanctions légales ou administratives
4. **Fonctionnalité de scan** : L'exactitude de la reconnaissance automatique de texte (OCR) lors du scan des étiquettes
5. **Disponibilité** : L'interruption, la suspension ou la cessation du service

### 4.5 Utilisation à vos risques

**VOUS UTILISEZ CETTE APPLICATION À VOS PROPRES RISQUES.** En cas de doute sur un produit, consultez impérativement :
- La base de données officielle E-Phy : https://ephy.anses.fr/
- Un conseiller agricole agréé
- Les services de la DRAAF (Direction Régionale de l'Alimentation, de l'Agriculture et de la Forêt)
- Un distributeur agréé de produits phytosanitaires

## 5. Obligations de l'utilisateur

### 5.1 Utilisation conforme

Vous vous engagez à :
- Utiliser l'application conformément à la législation en vigueur
- Ne pas utiliser l'application comme unique source d'information pour des décisions importantes
- Vérifier systématiquement les informations dans la base E-Phy officielle
- Respecter la réglementation française sur l'utilisation des produits phytosanitaires

### 5.2 Réglementation applicable

L'utilisation de produits phytosanitaires est strictement réglementée en France. Vous devez :
- Posséder le **Certiphyto** (certificat individuel pour l'utilisation professionnelle de produits phytosanitaires) si requis
- Respecter les conditions d'utilisation mentionnées sur les étiquettes des produits
- Éliminer les PPNU (Produits Phytosanitaires Non Utilisables) conformément à la réglementation
- Respecter les zones de non-traitement (ZNT)
- Tenir un registre phytosanitaire si vous êtes professionnel

### 5.3 Interdictions

Il est strictement interdit de :
- Utiliser l'application pour des activités illégales
- Modifier, décompiler ou tenter d'extraire le code source de l'application
- Revendre ou redistribuer l'application ou ses données
- Utiliser l'application pour porter atteinte à des tiers

## 6. Propriété intellectuelle

### 6.1 Droits d'auteur

L'application PhytoCheck, son code source, son design et son contenu (hors base de données E-Phy) sont la propriété exclusive de Franck Courrèges et sont protégés par les lois françaises et internationales sur la propriété intellectuelle.

### 6.2 Base de données E-Phy

La base de données E-Phy est une œuvre publique appartenant à l'État français. Son utilisation dans PhytoCheck est conforme aux conditions de réutilisation des données publiques.

### 6.3 Licence d'utilisation

Vous bénéficiez d'une licence non exclusive, non transférable et révocable pour utiliser l'application à des fins personnelles ou professionnelles.

## 7. Abonnement Premium

### 7.1 Fonctionnalités

L'abonnement Premium offre :
- Recherches illimitées (au lieu de 20 gratuites)
- Stock illimité (au lieu de 20 produits)
- Fonctionnalités futures réservées aux abonnés Premium

### 7.2 Tarifs et paiement

Les abonnements sont disponibles en :
- **Mensuel** : Paiement récurrent chaque mois
- **Annuel** : Paiement récurrent chaque année

Les paiements sont gérés par Google Play Billing. Les prix sont affichés dans l'application et peuvent varier selon votre pays.

### 7.3 Renouvellement automatique

Les abonnements se renouvellent automatiquement sauf annulation au moins 24 heures avant la fin de la période en cours. Vous pouvez annuler votre abonnement à tout moment depuis les paramètres de votre compte Google Play.

### 7.4 Remboursement

Les abonnements ne sont pas remboursables, sauf obligation légale. Conformément à la réglementation européenne, vous disposez d'un droit de rétractation de 14 jours pour les achats en ligne, sauf si vous avez commencé à utiliser le service.

### 7.5 Modification des tarifs

Les tarifs peuvent être modifiés à tout moment. Les modifications s'appliqueront au prochain cycle de facturation. Vous serez informé au moins 30 jours à l'avance.

## 8. Suspension et résiliation

### 8.1 Par l'utilisateur

Vous pouvez cesser d'utiliser l'application à tout moment en la désinstallant de votre appareil.

### 8.2 Par le développeur

Nous nous réservons le droit de suspendre ou de résilier votre accès à l'application en cas de :
- Violation des présentes CGU
- Utilisation frauduleuse ou abusive
- Activités illégales

### 8.3 Cessation du service

Nous nous réservons le droit de cesser définitivement le service PhytoCheck à tout moment, avec un préavis de 30 jours si possible.

## 9. Modifications des CGU

Ces Conditions Générales d'Utilisation peuvent être modifiées à tout moment. Les modifications entreront en vigueur dès leur publication. Il est de votre responsabilité de consulter régulièrement ces CGU. L'utilisation continue de l'application après modification vaut acceptation des nouvelles conditions.

## 10. Droit applicable et juridiction

### 10.1 Loi applicable

Les présentes CGU sont régies par le droit français.

### 10.2 Résolution des litiges

En cas de litige, nous vous encourageons à nous contacter pour tenter de trouver une solution amiable. À défaut, les tribunaux français seront seuls compétents.

## 11. Dispositions diverses

### 11.1 Intégralité de l'accord

Les présentes CGU constituent l'intégralité de l'accord entre vous et le développeur concernant l'utilisation de PhytoCheck.

### 11.2 Divisibilité

Si une disposition des présentes CGU est jugée invalide ou inapplicable, les autres dispositions resteront en vigueur.

### 11.3 Renonciation

Le fait de ne pas exercer un droit prévu par les présentes CGU ne constitue pas une renonciation à ce droit.

## 12. Contact

Pour toute question concernant ces Conditions Générales d'Utilisation, vous pouvez contacter :

**Franck Courrèges**  
Email : [votre-email@example.com]  
Site web : https://github.com/elkaou/phytocheck-app

## 13. RAPPEL IMPORTANT

**⚠️ PHYTOCHECK EST UN OUTIL D'AIDE À LA DÉCISION. VOUS DEVEZ IMPÉRATIVEMENT VÉRIFIER TOUTES LES INFORMATIONS DANS LA BASE DE DONNÉES OFFICIELLE E-PHY (https://ephy.anses.fr/) AVANT TOUTE UTILISATION DE PRODUITS PHYTOSANITAIRES.**

**⚠️ LE DÉVELOPPEUR NE PEUT ÊTRE TENU RESPONSABLE DE TOUTE CONSÉQUENCE RÉSULTANT DE L'UTILISATION DE CETTE APPLICATION.**

**⚠️ EN CAS DE DOUTE, CONSULTEZ UN PROFESSIONNEL QUALIFIÉ.**

---

*En utilisant PhytoCheck, vous reconnaissez avoir lu, compris et accepté ces Conditions Générales d'Utilisation.*

*Version 1.0.0 - 20 février 2026*
